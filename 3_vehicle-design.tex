...
\subsubsection{Cart3D Simulations}
...
For engine-off aerodynamics, the aerodynamic coefficients of the entire SPARTAN are extracted. However, for the engine-on aerodynamics of the SPARTAN, the engine flowpath and boat-tail of the SPARTAN are removed when the aerodynamic coefficients are extracted. The flowpath of the scramjet engines is assumed to be replaced by the conditions given by the C-RESTM10 engine database, and a separate Cart3D simulation is used to calculate the aerodynamic forces on the boat tail. 
...
\subsubsection{Engine-On Aerodynamic Analysis}\label{sec:engine-on}
...
\end{figure}
		...
		\subsubsection{Calculation of the Flap Deflections Necessary for Trim}\label{sec:trim}
		...
		Figure \ref{fig:FlapDeflection} shows the necessary flap deflections to trim the SPARTAN. Engine-on cases are shown at full-fuel, full conical tank, and empty conditions with the third stage included, and an engine-off case is shown at a a fuel-empty condition after third stage release. 
		...
		\textcolor{red}{be explicit about how moments are balanced with equation}
