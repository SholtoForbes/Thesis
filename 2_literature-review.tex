...
\chapter{Literature review}\label{chapter:literature-review}
  ...
  A Scramjet, or supersonic combustion ramjet, is an airbreathing engine design which combusts air at supersonic speeds and is capable of high Mach number operation. 
      ...
      Though scramjets are still in-development, the potential advantages which they offer over rockets for certain applications are driving their development in a number of institutions[CITATIONXX-Scramjet Propulsion].
  
  
  Scramjets compress air without moving parts, using geometry changes within the engine \cite{Curran2001a}, as well as on the forebody of the vehicle to create inlet shocks which provide the compression required for combustion\cite{Smart2012}. This is similar in operation to a ramjet engine, though a scramjet does not generate a normal shock, allowing supersonic air to enter the combustor. Maintaining supersonic speeds throughout the engine allows scramjets to operate efficiently at Mach numbers of 5 and greater. 
  
 
  
  
 It has been suggested that theoretically, scramjets are able to operate from Mach 4 all the way to Mach XX[CITATIONXX]. However, the operating range for a practical scramjet engine design is far more limited. 
 For a particular scramjet-powered vehicle, the operable region of the scramjet engines is constrained by the geometries of the forebody of the vehicle, and the inlet and combustor of the scramjet engine[CITATIONXX - Scramjets-smart]. Typically, scramjet engines are designed to operate from Mach numbers of XX-XX up to Mach numbers of XX-XX. The operable range of scramjet engines can be improved with mechanisms to vary the geometry of the inlet during flight[CITATIONXX ]. However, the systems necessary for variable geometry inlets add weight and complexity to the scramjet engine, and can be detrimental to overall system performance\cite{Smart2010}.
 
  Over the operational range of a scramjet engine, the specific impulse varies with velocity. 
  ...
  The high efficiency of scramjet engines means that significantly less propellant is used during flight compared to rocket engines, and consequently, that a much smaller fraction of a scramjet-powered vehicle consists of fuel mass. 
  The small fuel mass fraction of a vehicle powered by scramjet engines mitigates the mass of the vehicle systems, allowing features such as wings, control surfaces, landing gear, and passengers to be included in the vehicle design. 
  Missiles using scramjets are in development or in use by a number of nations, and passenger transport systems and satellite launch vehicles have been proposed which utilise scramjet engines.
  ...
  This has driven a large portion of the space flight industry to move towards privatisation, with a heavy focus on reusable technology. 
     ...
     Reusing launch vehicles allows the cost-over-time of the reused components to be reduced drastically, which subsequently allows the cost of individual launches to be reduced. Reduced costs minimises the barrier of entry for space launches, improving the diversity of space-based enterprises. 
  ...
  Reusable launch technology is the focus of many of the largest private launch companies. The SpaceX Falcon 9 and Falcon Heavy have been demonstrated on multiple occasions, landing booster stages successfully, and re-flying reused boosters multiple times. In the near future the Blue Origin New Glenn is planned, with potentially the Airbus Adeline to follow (to be used on the Ariane 6). 
  
  For a launch vehicle to be reusable, it must necessarily have the ability to come back to Earth safely, without damage to major system components. This return flight requires the addition of system components which allow the reusable stage to fly to a specified landing point. Control surfaces, structural components, additional fuel, and in the case of the Adeline, additional engines, must be incorporated within a reusable launch vehicle design. The additional weight that these components contribute further increases the fuel and structural mass necessary to initially accelerate the reusable stage. The impact of reusability on the mass and cost of the vehicle is minimised when the stage is larger, and when the velocity at the initiation of the return trajectory is decreased. Because of this mass increase on any stage which is to be designed to be reusable, most current reusable launch vehicle designs include only reusable first stages, with later stages being expendable. 
...
\caption{http://www.dailymail.co.uk/sciencetech/article-5361789/Ride-Elon-Musks-Starman-travels-space.html}
...
\end{figure}
  ...
  The majority of the small satellite launchers in development are expendable[CITATIONSXX]. These expendable small satellite launch systems aim to reduce costs by creating a launch system which is as cost-efficient as possible to produce and launch[CITATIONXX]. This generally entails making use of conventional, well-tested designs, combined with state of the art manufacturing techniques, such as 3-D printing[CITATIONXX]. This method allows for rapid development, although it has an intrinsic cost limit due to the requirement of manufacturing a new launch system for each launch.  
  
  Reusable small satellite launchers have far higher initial development costs, but also have the potential for large advantages in the long term. Reusable small satellite launchers have the potential for lower cost-per-launch than expendable systems, with increased launch flexibility[CITATIONXX]. 
   However, due to the structural mass fraction of a rocket stage increasing as the gross mass decreases, reusability will have a larger effect on a small launch system[CITATIONXX]. 
  The increased relative mass penalty for reusable systems on small launchers means that novel approaches are required for reusability[CITATIONSXX]. 
   It is notable that the launchers with re-use capabilities are entirely within the early stages of development, with no currently projected completion dates[CITATIONSXX]. 
   

  
  
  \textcolor{red}{Add in Falcon 1/1-e info.}
  ...
  \section{Airbreathing Access to Space Systems}
    The addition of a scramjet-powered stage to a small satellite launch system may allow for 
    
  
  -need to generalise this section and add more
...
Figure \ref{fig:AirbreathingCorridor} shows the operating corridor for scramjet engines, indicating the point at which transition to rocket stage would occur, the lower dynamic pressure limit on engine operation and the upper dynamic pressure limit on the aircraft structure.
\begin{figure}[ht]
	\centering
...
\subsection{Partially-Airbreathing Launch Vehicle Ascent Trajectories}
...
\subsubsection{Single-Stage Vehicles}
...
\end{figure}
 ...
 \subsubsection{Multi-Stage Vehicles}
...
\subsection{Hypersonic Vehicle Flyback Trajectories}
...
\textcolor{red}{Much more info, how is off design performance calculated, what model is used.}
