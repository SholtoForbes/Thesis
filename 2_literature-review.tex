...
GPOPS-2 is an hp-adaptive pseudospectral method solver designed for general optimal control problems. GPOPS-2 implements a variety of hp-adaptive pseudospectral methods, so that the best method may be chosen for a given problem. GPOPS-2 uses IPOPT (Interior Point OPTimizer) as standard nonlinear programming solver. IPOPT is an open source nonlinear optimisation package which utilises an interior point line search filter method.

GPOPS-2 is well proven in aerospace applications. 



https://link.springer.com/content/pdf/10.1007%2Fs10107-004-0559-y.pdf
...
There are a variety of tools available to calculate the aerodynamics of aerospace vehicles.
 These tools are primarily designed towards either accuracy or efficiency, as more accurate methods use more computational power. 
...
Low fidelity methods include the use of empirical relations derived from databases of existing vehicles[citeXX missile datcom], as well as panel method codes[cite cbaero, cart, hotsose].
Medium fidelity methods consist of inviscid Euler solvers [cite cart3d, AIRPLANE], which are able to provide reasonable accuracy, with medium run times. These solvers are often used in the later stages of preliminary design, or when higher fidelity is necessary due to design features. 
High fidelity methods mostly consist of Navier-Stokes CFD solvers. 

\textcolor{red}{Aerodynamic 	Analysis	of the JAPHAR Vehicle - says that HOTSOSE (similar to HYPAERO), is not suitable for simulating engine through-flow - reasoning for CART3D use}
