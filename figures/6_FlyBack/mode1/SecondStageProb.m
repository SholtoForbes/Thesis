...
for i = 1:length(q_vars)
    namelist{i} = ['qmax' num2str(q_vars(i)/1000)];
end
...
[auxdata.interp.Cl_spline_EngineOff.fullFuel,auxdata.interp.Cd_spline_EngineOff.fullFuel,...
    ...
    auxdata.T_spline_Rear,auxdata.Fd_spline_NoEngine] = AeroInt(aero1,auxdata,T_L,CG_z);
...
% due to fuel after the end of acceleration phase. 
...
bounds.phase(2).state.upper = [Stage2.Bounds.Alt(2), lonMax, latMax2, Stage2.Bounds.v(2), deg2rad(15), Stage2.Bounds.zeta(2), aoaMax21, bankMax21, Stage2.Bounds.mFuel(2)];
...
bounds.phase(3).finalstate.upper = [altMax, lon0, lat0, speedMax, fpaMax, aziMax, aoaMax21, bankMax21, mFuelMax, throttleMax];
end
% Control Bounds
...
mesh.maxiterations = 12;
mesh.colpointsmin = 8;
mesh.colpointsmax = 600;
mesh.tolerance    = 1e-5;
...
setup_par(i).guess.phase(2).state(:,1)   = [24000;24000 + 500*i]; % vary altitude guess
end
...
Plotter(output,auxdata,mode,returnMode,namelist,M_englist,T_englist,engine_data,MList_EngineOn,AOAList_EngineOn,mRocket,mSpartan,mFuel,h0,v0,bounds);
