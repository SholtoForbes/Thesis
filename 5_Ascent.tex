...
\begin{table}[ht]
	...
	\begin{tabular}{l c } 
		...
		& \textbf{\firstExergyDestroyedConstqNoReturn}
		\\
		...
		& \textbf{\secondExergyDestroyedConstqNoReturn}
		\\
		...
		& \textbf{\thirdExergyDestroyedConstqNoReturn}
		\\
...
The first stage rocket achieves an exergy efficiency of \firstExergyEffConstqNoReturn  XX\%$\eta$ when separating the SPARTAN onto a constant dynamic pressure trajectory. 
...
This section presents the maximum payload-to-orbit trajectory for the rocket-scramjet-rocket launch system, with the SPARTAN able to deviate from fixed dynamic pressure. 
...
\begin{tabular}{l c } 
	...
	\\
	\textbf{1$^{st}$ Stage Destroyed Exergy (GJ)}
	& \textbf{\firstExergyDestroyedStandardNoReturn}
	\\
	\textbf{Separation Alt, 1$\rightarrow$2 (km)}
	...
	\\
	\textbf{2$^{nd}$ Stage Destroyed Exergy (GJ)}
	& \textbf{\secondExergyDestroyedStandardNoReturn}
	\\
	\textbf{Separation Alt, 2$\rightarrow$3 (km)}
	...
	\\
	\textbf{3$^{rd}$ Stage Destroyed Exergy (GJ)}
	& \textbf{\thirdExergyDestroyedStandardNoReturn}
	\\
	\textbf{3$^{rd}$ Stage $t$, $q >$ 5kpa (s)}
...
increases the combined total efficiency launch system from \totalExergyEffqconstrained  XX\%$\eta$ to \totalExergyEffStandardNoReturn  XX\%$\eta$ (calculated including the energy lost due to stage separations). This is a relatively minor variation, and the payload-to-orbit benefits of this altitude raising manoeuvre are correspondingly small. 
...
Overall, the altitude raising manoeuvres which the SPARTAN performs result in a decrease in the exergy efficiency of the SPARTAN to \secondExergyEffStandardNoReturn  XX\%$\eta$, a total decrease of 1.318 XX\%$\eta$ compared to the SPARTAN flying at a constant dynamic pressure. However, the optimised trajectory drastically increases the exergy efficiency of the third stage, to \thirddExergyEffStandardNoReturn  XX\%$\eta$, an overall increase of \textcolor{red}{XX6.72  XX\%$\eta$} compared to the third stage released from the SPARTAN flying a fixed dynamic pressure trajectory.  
This leads to the total exergy efficiency of the launch system increasing from \totalExergyEffConstqNoReturn  XX\%$\eta$ to \totalExergyEffStandardNoReturn  XX\%$\eta$. 
...
\begin{table}[ht]
	...
	\begin{tabular}{l c c} 
		...
		&No $q$ Constraint
		\\
		\textbf{Total First Stage Exergy} 
		&\textbf{\firstEnergyConstqNoReturn} GJ
		&\textbf{\firstEnergyStandardNoReturn} GJ
		\\
		
		\textcolor{blue}{KE + PE of Payload}
		& \firstWpayloadConstqNoReturn \%
		& \firstWpayloadStandardNoReturn \%
		\\
		\textcolor{red}{KE + PE of  2$^{nd}$ \& 3$^{rd}$ Stage}
		& \firstWnextStageConstqNoReturn \% & \firstWnextStageStandardNoReturn \%
		\\
		
		\textcolor{red}{Overcoming Drag} 
		& \WDoneConstqNoReturn \% & \WDoneStandardNoReturn \%
		\\
		\textcolor{red}{KE + PE of 1$^{st}$ Stage Structural Mass} 
		& \WoneConstqNoReturn \% & \WoneStandardNoReturn \%
		\\
		\textcolor{red}{Propulsion Inefficiency} 
		& \PlossoneCombinedConstqNoReturn \% & \PlossoneCombinedStandardNoReturn \%
		\\ 
		\textbf{Total SPARTAN Exergy} 
		& \textbf{\secondEnergyConstqNoReturn} GJ & \textbf{\secondEnergyStandardNoReturn} GJ
		\\
		\textcolor{blue}{KE + PE of Payload}
		& \secondWpayloadConstqNoReturn \% & \secondWpayloadStandardNoReturn \%
		\\
		\textcolor{red}{KE + PE of 3$^{rd}$ Stage}
		& \secondWnextStageConstqNoReturn \% & \secondWnextStageStandardNoReturn \%
		\\
		\textcolor{red}{Overcoming Drag}
		& \WDsecondConstqNoReturn \% & \WDsecondStandardNoReturn \%
		\\
		\textcolor{red}{KE + PE of SPARTAN Structural Mass}  
		& \WsecondConstqNoReturn \% & \WsecondStandardNoReturn \%
		\\
		\textcolor{red}{Propulsion Inefficiency}  
		& \PlosssecondCombinedConstqNoReturn \% & \PlosssecondCombinedStandardNoReturn \%
		\\
		
		\textbf{Total Third Stage Exergy}  
		& \textbf{\thirdEnergyConstqNoReturn} GJ & \textbf{\thirdEnergyStandardNoReturn} GJ
		\\
		\textcolor{blue}{KE + PE of Payload}  
		&\thirddExergyEffConstqNoReturn \% &\thirddExergyEffStandardNoReturn \%
		\\
		\textcolor{red}{Overcoming Drag}  
		& \WDthreeConstqNoReturn \% & \WDthreeStandardNoReturn \%
		\\
		\textcolor{red}{KE + PE  of 3$^{rd}$ Stage Structural Mass}  
		& \WthreeConstqNoReturn \% & \WthreeStandardNoReturn \%
		\\
		
		\textcolor{red}{KE + PE of Heat Shield}  
		
		& \WHSthreeConstqNoReturn \% & \WHSthreeStandardNoReturn \%
		\\
		
		\textcolor{red}{Propulsion Inefficiency}  
		& \PlossthreeCombinedConstqNoReturn \% & \PlossthreeCombinedStandardNoReturn \%
		\\
		\hline 
	\end{tabular} 
	\caption{An exergy usage breakdown of the ascent trajectories, both with, and without, the SPARTAN constrained to constant dynamic pressure (Cases 1 \& 2). Blue indicates a 'productive' exergy usage, whereas red indicates exergy 'wastage'.}
	\label{tab:effStandardNoReturn}
\end{table}



An exergy analysis is conducted on the maximum payload-to-orbit launch trajectories, both with, and without, the SPARTAN constrained to constant dynamic pressure flight (Cases 1 \& 2). An exergy breakdown of each of each stage is compared in Table \ref{tab:effStandardNoReturn}. The exergy used to accelerate the payload is shown, along with the exergy wasted imparting energy to the successive stages; overcoming drag; imparting energy to the structural mass of each stage, which is separated; and lost due to propulsion inefficiency. 

From this exergy usage breakdown it can be observed that the exergy utilised by the first stage and the SPARTAN to impart exergy to the payload is relatively low, compared to the exergy of the third stage utilised for this purpose, which is significantly higher. This is due to the larger mass fraction of the payload during third stage acceleration, particularly during the Hohmann transfer, where large velocities are imparted, and the payload makes up a significant portion of the total mass. 
The SPARTAN uses more exergy accelerating the third stage when flying a maximum-payload-to-orbit trajectory with no dynamic pressure constraint, compared to a trajectory constrained to 50kPa, and at the same time uses imparts less exergy to the third stage. This is due to the trade-off between the payload and fuel of the third stage, which decreases the third stage fuel mass as the payload increases. 
The first stage rocket loses a significant portion of its exergy to propulsion inefficiency, at \PlossoneCombinedStandardNoReturn \% when flying a trajectory without a dynamic pressure constraint. This is due to the low propulsive efficiency of rockets at low speeds, which results from a large amount of kinetic energy being imparted onto the accelerated propellant, for very little acceleration of the launch system. 
This is higher than both the SPARTAN's  and the third stage's losses to propulsion inefficiency, at \PlosssecondCombinedStandardNoReturn \% and \PlossthreeCombinedStandardNoReturn \% respectively.
The SPARTAN has significantly lower propulsion losses due to its scramjet engines utilising atmospheric oxygen as oxidiser, resulting in a high propulsive efficiency. However, the SPARTAN loses \WDsecondStandardNoReturn \% of its exergy to overcoming drag. This high drag loss is due to the SPARTAN accelerating at high speeds within the atmosphere, and serves to partially offset the reduced exergy loss due to high propulsive efficiency. 
The propulsive energy losses of the third stage are also low, at \PlossthreeCombinedStandardNoReturn \%. This is due to the larger velocity of the third stage, compared to the first stage, which increases the propulsive efficiency of the rocket. 
The exergy lost from the third stage overcoming drag is dependant on the amount of time which the rocket spends in the atmosphere, and comprises \WDthreeConstqNoReturn \% when released at 50kPa, and \WDthreeStandardNoReturn \% when released after a pull-up of the SPARTAN.
The exergy lost accelerating the heat shield is also significantly larger when released from a constant dynamic pressure trajectory, at \WHSthreeConstqNoReturn \%, compared to only \WHSthreeStandardNoReturn \% when the third stage is released after a pull-up of the SPARTAN. This is due to the third stage spending considerably more time in a high dynamic pressure environment when released from a constant $q$ trajectory, requiring the heat shield for longer. 
...
The variation in maximum dynamic pressure has only a small effect on the total exergy efficiency of the system, and hence only a small effect on the payload mass delivered to sun synchronous orbit.  Varying the maximum dynamic pressure by $\pm20\%$ causes a variation of only +0.065 XX\%$\eta$ or -0.069 XX\%$\eta$ in exergy efficiency and a corresponding +7.2kg (+3.8\%) or -7.8kg (-4.1\%) variation in payload to orbit.  
...
The drag of the SPARTAN has a significant effect on the overall exergy efficiency of the system (+0.182 XX\%$\eta$ at 90\% drag, and -0.147 XX\%$\eta$ at 110\% drag) and correspondingly, on the maximum payload-to-orbit, +19.6kg at 90\% drag, a variation of +10.4\% and -17.2kg at 110\% drag, a variation of -9.1\%. The exergy efficiencies of the first stage rocket and the SPARTAN are decreased significantly as the drag is increased, from \firstExergyEffCdNinetyNoReturn XX\%$\eta$ and \secondExergyEffCdNinetyNoReturn XX\%$\eta$ respectively at 90\% drag, to \firstExergyEffCdOneHundredTenNoReturn XX\%$\eta$ and \secondExergyEffCdOneHundredTenNoReturn XX\%$\eta$ respectively at 110\% drag. This reduction in efficiency is due to the increase in energy which must be used to overcome the added drag. 
...
\begin{table}[ht]
	...
	& $\Delta/\Delta$\%q
		\\
		\hline \textbf{Payload to Orbit (kg)}
		& \textbf{\PayloadToOrbitmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\PayloadToOrbitmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\PayloadToOrbitmSPARTANStandardNoReturnNoReturn}
		& \textbf{\PayloadToOrbitmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\PayloadToOrbitmSPARTANOneHundredFiveNoReturnNoReturn}
		&\textbf{-1.5}
		\\
		\textbf{Payload Variation (\%)}
		& \PayloadVarmSPARTANNinetyFiveNoReturnNoReturn
		& \PayloadVarmSPARTANNinetySevenFiveNoReturnNoReturn
		& \PayloadVarmSPARTANStandardNoReturnNoReturn
		& \PayloadVarmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \PayloadVarmSPARTANOneHundredFiveNoReturnNoReturn
		&-0.8
		\\
		\textbf{Total $\eta_{exergy}$ (\%)}
		& \textbf{\totalExergyEffmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\totalExergyEffmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\totalExergyEffmSPARTANStandardNoReturnNoReturn}
		& \textbf{\totalExergyEffmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\totalExergyEffmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{-0.00011}
		\\
		\hline 
		\textbf{1$^{st}$ Stage Destroyed Exergy (GJ)}
		& \textbf{\firstExergyDestroyedmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\firstExergyDestroyedmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\firstExergyDestroyedmSPARTANStandardNoReturnNoReturn}
		& \textbf{\firstExergyDestroyedmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\firstExergyDestroyedmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{-488100531.96}
		\\
		\textbf{Separation Alt, 1$\rightarrow$2 (km)}
		& \firstsecondSeparationAltmSPARTANNinetyFiveNoReturnNoReturn
		& \firstsecondSeparationAltmSPARTANNinetySevenFiveNoReturnNoReturn
		& \firstsecondSeparationAltmSPARTANStandardNoReturnNoReturn
		& \firstsecondSeparationAltmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \firstsecondSeparationAltmSPARTANOneHundredFiveNoReturnNoReturn
		&-0.22
		\\
		\textbf{Separation v, 1$\rightarrow$2 (m/s)}
		& \firstsecondSeparationvmSPARTANNinetyFiveNoReturnNoReturn
		& \firstsecondSeparationvmSPARTANNinetySevenFiveNoReturnNoReturn
		& \firstsecondSeparationvmSPARTANStandardNoReturnNoReturn
		& \firstsecondSeparationvmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \firstsecondSeparationvmSPARTANOneHundredFiveNoReturnNoReturn
		&-11.97
		\\
		\textbf{Separation $\gamma$, 1$\rightarrow$2 (deg)}
		& \firstsecondSeparationgammamSPARTANNinetyFiveNoReturnNoReturn
		& \firstsecondSeparationgammamSPARTANNinetySevenFiveNoReturnNoReturn
		& \firstsecondSeparationgammamSPARTANStandardNoReturnNoReturn
		& \firstsecondSeparationgammamSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \firstsecondSeparationgammamSPARTANOneHundredFiveNoReturnNoReturn
		&-0.34
		\\
		\hline 
		\textbf{2$^{nd}$ Stage Destroyed Exergy (GJ)}
		& \textbf{\secondExergyDestroyedmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\secondExergyDestroyedmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\secondExergyDestroyedmSPARTANStandardNoReturnNoReturn}
		& \textbf{\secondExergyDestroyedmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\secondExergyDestroyedmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{18590973.409}
		\\
		\textbf{Separation Alt, 2$\rightarrow$3 (km)}
		& \secondthirdSeparationAltmSPARTANNinetyFiveNoReturnNoReturn
		& \secondthirdSeparationAltmSPARTANNinetySevenFiveNoReturnNoReturn
		& \secondthirdSeparationAltmSPARTANStandardNoReturnNoReturn
		& \secondthirdSeparationAltmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \secondthirdSeparationAltmSPARTANOneHundredFiveNoReturnNoReturn
		& -
		\\
		\textbf{Separation $v$, 2$\rightarrow$3 (m/s)}
		& \secondthirdSeparationvmSPARTANNinetyFiveNoReturnNoReturn
		& \secondthirdSeparationvmSPARTANNinetySevenFiveNoReturnNoReturn
		& \secondthirdSeparationvmSPARTANStandardNoReturnNoReturn
		& \secondthirdSeparationvmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \secondthirdSeparationvmSPARTANOneHundredFiveNoReturnNoReturn
		&-9.54
		\\
		\textbf{Separation $\gamma$, 2$\rightarrow$3 (deg)}
		& \secondthirdSeparationgammamSPARTANNinetyFiveNoReturnNoReturn
		& \secondthirdSeparationgammamSPARTANNinetySevenFiveNoReturnNoReturn
		& \secondthirdSeparationgammamSPARTANStandardNoReturnNoReturn
		& \secondthirdSeparationgammamSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \secondthirdSeparationgammamSPARTANOneHundredFiveNoReturnNoReturn
		&0.06
		\\
		\textbf{2$^{nd}$ Stage Flight Time (s)}
		& \secondFlightTimemSPARTANNinetyFiveNoReturnNoReturn
		& \secondFlightTimemSPARTANNinetySevenFiveNoReturnNoReturn
		& \secondFlightTimemSPARTANStandardNoReturnNoReturn
		& \secondFlightTimemSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \secondFlightTimemSPARTANOneHundredFiveNoReturnNoReturn
		& -
		\\
		\textbf{2$^{nd}$ Stage Distance Flown (km)}
		& \SecondDistmSPARTANNinetyFiveNoReturnNoReturn
		& \SecondDistmSPARTANNinetySevenFiveNoReturnNoReturn
		& \SecondDistmSPARTANStandardNoReturnNoReturn
		& \SecondDistmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \SecondDistmSPARTANOneHundredFiveNoReturnNoReturn
		&-10.68
		\\
		\hline 
		\textbf{3$^{rd}$ Stage Destroyed Exergy (GJ)}
		& \textbf{\thirdExergyDestroyedmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\thirdExergyDestroyedmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\thirdExergyDestroyedmSPARTANStandardNoReturnNoReturn}
		& \textbf{\thirdExergyDestroyedmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\thirdExergyDestroyedmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{65200128.394}
		\\
		\textbf{3$^{rd}$ Stage $t$, $q >$ 5kpa (s)}
		& \thirdqOverFivemSPARTANNinetyFiveNoReturnNoReturn
		& \thirdqOverFivemSPARTANNinetySevenFiveNoReturnNoReturn
		& \thirdqOverFivemSPARTANStandardNoReturnNoReturn
		& \thirdqOverFivemSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \thirdqOverFivemSPARTANOneHundredFiveNoReturnNoReturn
		&-0.24
		\\
		\textbf{3$^{rd}$ Stage max $\alpha$ (deg)}
		& \thirdmaxAoAmSPARTANNinetyFiveNoReturnNoReturn
		& \thirdmaxAoAmSPARTANNinetySevenFiveNoReturnNoReturn
		& \thirdmaxAoAmSPARTANStandardNoReturnNoReturn
		& \thirdmaxAoAmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \thirdmaxAoAmSPARTANOneHundredFiveNoReturnNoReturn
		& -
		\\
		\textbf{3$^{rd}$ Stage Fuel Mass (kg)}
		& \thirdmFuelmSPARTANNinetyFiveNoReturnNoReturn
		& \thirdmFuelmSPARTANNinetySevenFiveNoReturnNoReturn
		& \thirdmFuelmSPARTANStandardNoReturnNoReturn
		& \thirdmFuelmSPARTANOneHundredTwoFiveNoReturnNoReturn
		& \thirdmFuelmSPARTANOneHundredFiveNoReturnNoReturn
		&1.52
		\\
		\hline 
	\end{tabular}  
...
The exergy efficiency of the first stage rocket decreases as the mass of the SPARTAN is increased (-0.300 XX\%$\eta$  at 105\% structural mass) and increases as the mass of the SPARTAN is decreased (+0.309 XX\%$\eta$  at 95\% structural mass). As the mass of the SPARTAN increases, the acceleration of the first stage decreases, and the first stage spends a larger time in high density conditions, causing the efficiency of the system to be reduced.
Additionally, the altitude and trajectory angle at first stage-SPARTAN separation are decreased significantly. This change in separation conditions, along with the decreased exergy efficiency of the first stage, indicates that as the mass of the SPARTAN increases, it becomes beneficial to trade-off more of the exergy efficiency of the first stage, to benefit the performance of the SPARTAN \textcolor{red}{XX see ingos note, check this}. This variation in the optimal trade-off is due in part to the first stage being able to pitch more easily with added SPARTAN mass, so that it is physically able to achieve a wider range of separation conditions more easily. Another factor which contributes to the variation in the optimal trade-off is the efficiency of the SPARTAN. As the SPARTAN's mass increases, the first stage-SPARTAN separation velocity decreases significantly. This results in the SPARTAN maintaining high specific impulse for a longer time at the beginning of its trajectory. As well as this initial increase in efficiency, a higher SPARTAN structural mass causes the SPARTAN to stay at relatively lower velocities over its trajectory, which results in a higher specific impulse throughout. The increased specific impulse of the SPARTAN means that trading-off the efficiency of the first stage for the efficiency of the SPARTAN is more beneficial. 
The increased efficiency of the C-REST engines, as well as the favourable trade-off from the first stage causes the exergy efficiency of the SPARTAN to rise as the structural mass increases (by +0.296 XX\%$\eta$ at 105\% mass) and to decrease as the structural mass decreases (by -0.333 XX\%$\eta$ at 95\% mass). However, as the SPARTAN mass is increased, more work must be used accelerating the additional mass, which is then separated, causing the overall efficiency of the launch system to decrease \textcolor{red}{not entirely correct, isp not = to efficiency increase. use exergy breakdown}. 
...
As the mass of the SPARTAN increases, the exergy efficiency of the third stage is increased, varying by +0.275 XX\%$\eta$ at 105\% structural mass, and as the SPARTAN mass is decreased, the exergy efficiency of the third stage also decreases, varying by -0.247 XX\%$\eta$ at 95\% structural mass. 
This indicates that the trade-off between the efficiency of the SPARTAN and the third stage rocket is shifted in favour of the third stage rocket as the exergy efficiency of the SPARTAN increases. 
This seemingly runs counter to the results of Cases 4 and 5, which both show the trade-off between the efficiencies of the SPARTAN and third stage shifting towards the SPARTAN as the exergy efficiency of the SPARTAN increases. 
\textcolor{red}{XX change this}
However, the exergy efficiency calculation for the SPARTAN does not take into account the energy wasted in accelerating the additional mass of the SPARTAN. This additional energy causes the overall exergy efficiency of the launch system to decrease as the mass of the SPARTAN increases. This additional mass effectively reduces the `useful' energy available to the SPARTAN, and causes the optimal trade-off between the exergy efficiencies of the SPARTAN and the third stage to shift towards the third stage. 
...
This diminishing specific impulse causes the exergy efficiency of the SPARTAN to decrease by -0.296 XX\%$\eta$ (-2.7\%) at 110\% fuel mass, and to increase by +0.318 XX\%$\eta$ (+2.9\%) at 90\% fuel mass. 
While the exergy efficiency decreases, the addition of extra fuel mass means that there is more total energy available (XX at XX compared to XX \textcolor{red}{see ingos notes here, maybe use exergy breakdown}), and so the SPARTAN is able to accelerate more over its trajectory. For this reason, the payload mass-to-orbit of the vehicle increases as extra fuel is added to the SPARTAN, even though the total exergy efficiency of the system decreases. 

The exergy efficiency of the third stage is decreased as the fuel mass is increased. 
This indicates that as the fuel mass of the SPARTAN is increased, the optimal performance trade-off between the SPARTAN and third stage rocket shifts to favour the SPARTAN. 
...
The total mass of the third stage rocket is varied by $\pm10\%$, to investigate the effects of variations in the internal mass density of the third stage rocket on the performance of the launch system. Table \ref{tab:comparison80} details key trajectory parameters, and Appendix \ref{sec:app_comparison80} presents comparison plots of each trajectory. The mass which is added or subtracted from the third stage is a combination of payload, fuel and structural mass. 
The mass of the heat shield is unchanged at 130.9kg, and the structural mass is assumed to contribute to 9\% of the remaining mass (so that the structural mass varies by $\pm10\%$). The remaining mass which is varied consists of a flexible combination of fuel and payload mass, ie. the optimiser may use the available mass as fuel, and the mass leftover when orbit is reached is assumed to be payload, in the same manner as all other cases. 
...
As the mass of the third stage is varied by $\pm10\%$, the exergy efficiency of the SPARTAN varies by +0.262 XX\%$\eta$ (+2.4\%) and -0.428 XX\%$\eta$ (-3.9\%) respectively, and the exergy efficiency of the third stage varies by +0.471 XX\%$\eta$ (+2.3\%) and -0.379 XX\%$\eta$ (-1.9\%) respectively. The exergy efficiency variation of the SPARTAN is due to the SPARTAN flying at lower velocity when the third stage mass is higher, resulting in higher specific impulse from the scramjet engines. 
...
As the specific impulse of the third stage is varied, the size and shape of the SPARTAN's trajectory and pull-up manoeuvre are only slightly affected by the specific impulse of the third stage rocket engine, with no consistent trend. 
 However, the exergy efficiency of the SPARTAN increases as the third stage specific impulse is increased, by +0.072 XX\%$\eta$ (+0.66\%) at 105\% $I_{SP}$, and decreases as the third stage specific impulse is decreased, by -0.035 XX\%$\eta$ (-0.032\%) at 95\% $I_{SP}$. This trend indicates that as the efficiency of the third stage increases, the trade-off between the efficiencies of the SPARTAN and the third stage shifts slightly to favour the SPARTAN. This trade-off shift is small, because the third stage specific impulse has a relatively minor effect of within the atmosphere, where the trade-off occurs. 
...
A trajectory optimised for maximum payload-to-orbit was then calculated, which increased payload mass to heliosynchronous orbit to \PayloadToOrbitStandard kg (an increase of 16.3\%) compared to the constant dynamic pressure trajectory.
  ...
  The optimal flight path then exhibits an altitude raising manoeuvre in the middle of the trajectory, which improves the exergy efficiency of the SPARTAN by a very minor +0.003 XX\%$\eta$ (+0.03\%). 
  Finally, the SPARTAN executes a pull-up manoeuvre before the SPARTAN-third stage separation. This optimal pull-up manoeuvre trades off velocity (a decrease of 116.2m/s) for altitude (an increase of 9.48km) and improved flight path angle (an increase of 10.45$^\circ$), decreasing the exergy efficiency of the SPARTAN by -1.32  XX\%$\eta$ (-10.7\%) when compared to the constant dynamic pressure case. 
  These conditions improve the exergy efficiency of the third stage rocket significantly, by 3.456 XX\%$\eta$, an increase of 20.5\% over the third stage released from a constant dynamic pressure trajectory. 
