...
\end{figure}
LODESTAR successfully computes the trajectory of the rocket-scramjet-rocket system, with the SPARTAN flying at constant dynamic pressure, achieving a payload-to-orbit of \PayloadToOrbitConstq kg.
...
The SPARTAN is separated at a trajectory angle of \firstsecondSeparationgammaConstq$^\circ$ at an altitude of \firstsecondSeparationAltConstq km, a total flight time of 120.9s, with a total ground distance of \FirstStageDistStandardNoReturn km covered under power of the first stage rocket. 
...
\begin{table}[ht]
	...
	\begin{tabular}{l c } 
		...
		\\
		\textcolor{blue}{Acceleration of Stage 2 \& 3}
		& \firstExergyEffStandardNoReturn \%
		\\
		\textcolor{red}{Overcoming Drag} 
		& \WDoneStandardNoReturn \%
		\\
		\textcolor{red}{Accelerating fuel}
		& \WmFoneStandardNoReturn \%
		\\
		\textcolor{red}{Acceleration of Structural Mass} 
		& \WoneStandardNoReturn \%
		\\
		\textcolor{red}{Propulsion Inefficiency} 
		& \PlossoneStandardNoReturn \%
		\\
			\textbf{Total SPARTAN Exergy} 
		& \textbf{\secondEnergyStandardNoReturn} GJ
		\\
		\textcolor{blue}{Acceleration of Stage 3} 
		& \secondExergyEffStandardNoReturn \%
		\\
		\textcolor{red}{Overcoming Drag}
		& \WDsecondStandardNoReturn \%
		\\
		\textcolor{red}{Propulsion Inefficiency}  
		& \PlosssecondStandardNoReturn \%
		\\
		\textcolor{red}{Accelerating fuel}  
		& \WmFsecondStandardNoReturn \%
		\\
		\textcolor{red}{Acceleration of Structural Mass}  
		& \WsecondStandardNoReturn \%
		\\
		\textbf{	Total Third Stage Exergy}  
		& \textbf{\thirdEnergyStandardNoReturn} GJ
		\\
		\textcolor{blue}{Accelerating Payload}  
		&\thirddExergyEffStandardNoReturn \%
		\\
		\textcolor{red}{Overcoming Drag}  
		& \WDthreeStandardNoReturn \%
		\\
		\textcolor{red}{Accelerating Structural Mass}  
		& \WthreeStandardNoReturn \%
		\\
		
		\textcolor{red}{Accelerating Fuel, In Atmosphere} 
		& \WmFthreeStandardNoReturn \%
		\\
		\textcolor{red}{Acceleration of Heat Shield}  
		
		& \WHSthreeStandardNoReturn \%
		\\
		
		\textcolor{red}{Propulsion Inefficiency, In Atmosphere}  
		& \PlossthreeStandardNoReturn \%
		\\
		\textcolor{red}{Losses During Hohmann Transfer}  
		
		& \HTlossStandardNoReturn \%
		\\
		\hline 
	\end{tabular} 
	\caption{\textcolor{red}{not just accelerating, change labels}A summary of key results from the maximum payload-to-orbit trajectory (Case 2).}
...
Overall, the altitude raising manoeuvres which the SPARTAN performs result in a decrease in the exergy efficiency of the SPARTAN to \secondExergyEffStandardNoReturn \%$\eta$, a total decrease of 1.318\%$\eta$ compared to the SPARTAN flying at a constant dynamic pressure. However, the optimised trajectory drastically increases the exergy efficiency of the third stage, to \thirddExergyEffStandardNoReturn \%$\eta$, an overall increase of \textcolor{red}{XX6.72 \%$\eta$} compared to the third stage released from the SPARTAN flying a fixed dynamic pressure trajectory.  
...
The variation in maximum dynamic pressure has only a small effect on the total exergy efficiency of the system, and hence only a small effect on the payload mass delivered to sun synchronous orbit.  Varying the maximum dynamic pressure by $\pm20\%$ causes a variation of only +0.065\%$\eta$ or -0.069\%$\eta$ in exergy efficiency and a corresponding +7.2kg (+3.8\%) or -7.8kg (-4.1\%) variation in payload to orbit.  
...
Increasing the specific impulse of the C-REST engines increases the exergy efficiency of the SPARTAN considerably, by +1.724\%$\eta$ (+15.8\%) at 110\%$I_{SP}$ and by -1.558\%$\eta$ (-14.2\%) at 90\% $I_{SP}$. 
...
\begin{table}[ht]
	...
	& $\Delta/\Delta$\%q
		...
		\hline 
		\textbf{1$^{st}$ Stage $\eta_{exergy}$ (\%)}
		& \textbf{\firstExergyEffmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\firstExergyEffmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\firstExergyEffmSPARTANStandardNoReturnNoReturn}
		& \textbf{\firstExergyEffmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\firstExergyEffmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{-0.043}
		\\
		\textbf{Separation Alt, 1$\rightarrow$2 (km)}
		...
		\hline 
		\textbf{2$^{nd}$ Stage $\eta_{exergy}$ (\%)}
		& \textbf{\secondExergyEffmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\secondExergyEffmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\secondExergyEffmSPARTANStandardNoReturnNoReturn}
		& \textbf{\secondExergyEffmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\secondExergyEffmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{-0.01}
		\\
		\textbf{Separation Alt, 2$\rightarrow$3 (km)}
		...
		\hline 
		\textbf{3$^{rd}$ Stage $\eta_{exergy}$ (\%)}
		& \textbf{\thirddExergyEffmSPARTANNinetyFiveNoReturnNoReturn}
		& \textbf{\thirddExergyEffmSPARTANNinetySevenFiveNoReturnNoReturn}
		& \textbf{\thirddExergyEffmSPARTANStandardNoReturnNoReturn}
		& \textbf{\thirddExergyEffmSPARTANOneHundredTwoFiveNoReturnNoReturn}
		& \textbf{\thirddExergyEffmSPARTANOneHundredFiveNoReturnNoReturn}
		& \textbf{-0.147}
		\\
		\textbf{3$^{rd}$ Stage $t$, $q >$ 5kpa (s)}
...
The exergy efficiency of the first stage rocket decreases as the mass of the SPARTAN is increased (-0.300\%$\eta$  at 105\% structural mass) and increases as the mass of the SPARTAN is decreased (+0.309\%$\eta$  at 95\% structural mass). As the mass of the SPARTAN increases, the acceleration of the first stage decreases, and the first stage spends a larger time in high density conditions, causing the efficiency of the system to be reduced.
...
As the mass of the SPARTAN increases, the exergy efficiency of the third stage is increased, varying by +0.275\%$\eta$ at 105\% structural mass, and as the SPARTAN mass is decreased, the exergy efficiency of the third stage also decreases, varying by -0.247\%$\eta$ at 95\% structural mass. 
...
As the mass of the third stage is varied by $\pm10\%$, the exergy efficiency of the SPARTAN varies by +0.262\%$\eta$ (+2.4\%) and -0.428\%$\eta$ (-3.9\%) respectively, and the exergy efficiency of the third stage varies by +0.471\%$\eta$ (+2.3\%) and -0.379\%$\eta$ (-1.9\%) respectively. The exergy efficiency variation of the SPARTAN is due to the SPARTAN flying at lower velocity when the third stage mass is higher, resulting in higher specific impulse from the scramjet engines. 
...
As the specific impulse of the third stage is varied, the size and shape of the SPARTAN's trajectory and pull-up manoeuvre are only slightly affected by the specific impulse of the third stage rocket engine, with no consistent trend. 
 However, the exergy efficiency of the SPARTAN increases as the third stage specific impulse is increased, by +0.072\%$\eta$ (+0.66\%) at 105\% $I_{SP}$, and decreases as the third stage specific impulse is decreased, by -0.035\%$\eta$ (-0.032\%) at 95\% $I_{SP}$. This trend indicates that as the efficiency of the third stage increases, the trade-off between the efficiencies of the SPARTAN and the third stage shifts slightly to favour the SPARTAN. This trade-off shift is small, because the third stage specific impulse has a relatively minor effect of within the atmosphere, where the trade-off occurs. 
